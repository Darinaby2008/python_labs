def format_record(student: tuple[str, str, float]) -> str:
    ...
    if not (
        isinstance(student[0], str)
        and isinstance(student[1], str)
        and isinstance(student[2], float)
    ):  # проверяем, что именно 1-фио, 2-группа,3-GPA.
        return "TypeError"
    ...
    res = (
        fio_parts[0].title() + " " + fio_parts[1][0].upper()
    )  # Формируем фамилию и первую букву имени с большой буквы
    ...
    res += (
        "гр. " + student[1] + ", GPA " + f"{round(student[2],2):.2f}"
    )  # соединяем все в одно и округляем
    return res
